...
from bert_embeddings import BERTChunkEmbeddings


def build_database(dataset_path, dataset_file_name, chunk_len: int = 16, batch_size: int = 64, d_emb: int = 768, 
                   n_centeroids: int = 256, code_size: int = 64, n_probe: int = 8, 
                   n_train: int = 50_000):
    """
    ## Build Database
    ...
    * `n_train' is the number of keys to train the index on
    ...
    dataset = TextFileDataset(
        Path(dataset_path/dataset_file_name),
        list)
    ...
    random_sample = np.random.choice(np.arange(n_chunks), size=[min(n_train, n_chunks)], 
                                     replace=False)
    ...
    for s in range(0, n_chunks, 1024):
        e = min(s + 1024, n_chunks)
        # Add to index
        index.add_with_ids(chunk_emb[s:e], chunk_offsets[s: e])

    # Save the index
    faiss.write_index(index, str(dataset_path / 'retro.index'))
...
class RetroIndex:
    ...
    def __init__(self, dataset_path, dataset_file_name, chunk_len: int = 16, n_probe: int = 8,
        ...
        * `exclude_neighbor_span` is the extra text length to avoid when checking for overlaps
        ...
        # Initialize BERT to get $\text{B\small{ERT}}(N)$
        self.bert = BERTChunkEmbeddings(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
        # Load the database
        self.index = faiss.read_index(str(dataset_path / 'retro.index'))
...
if __name__ == '__main__':
    """
    In Memory:
    1. List of chunked strings
    2. List of starting indices of the above chunked strings 
    3. k-Neighbor offsets
